...
/* Which animals each language draws on. They are the same set today; the pools
   are separate so German can grow without waiting for English fragments. */
const MIX_POOL = {
  de: ["krokodil", "elefant", "jaguar", "giraffe", "flamingo", "gorilla"],
  en: ["krokodil", "elefant", "jaguar", "giraffe", "flamingo", "gorilla"]
};
const canAnswer = (k, slot, lang) => (TIERE[k][lang][slot] || "").length >= MIX_MIN_FRAG;
...
const TIER_ORDER = ["krokodil", "elefant", "jaguar", "giraffe", "flamingo", "gorilla"];
const TIER_IMG = {
...
const mixOptions = (answer, slot, lang, rnd) => {
  ...
  const partner = MIX_POOL[lang]
    .filter((k) => k !== answer && canAnswer(k, slot, lang) && TIERE[k][lang][slot] !== right)
    .map((k) => ({ k, s: fragScore(TIERE[k][lang][slot], right) }))
    .sort((x, y) => y.s - x.s)[0];
  const tiles = [{ animal: answer, frag: right, ok: true }];
  const pf = partner ? TIERE[partner.k][lang][slot] : null;
  ...
  while (tiles.length < MIX_TILES) {
    const spare = MIX_POOL[lang].filter((k) =>
      canAnswer(k, slot, lang) && !tiles.some((t) => t.frag === TIERE[k][lang][slot]));
    if (!spare.length) break;
    const k = spare[Math.floor(rnd() * spare.length)];
    tiles.push({ animal: k, frag: TIERE[k][lang][slot] });
  }
...
/* One round: six creatures with a single slot open, then two creatures with
   ...
   otherwise the creature the mode is named after is a 1-in-216 accident he
